...
async function generateStructuredAnalysis(ai, commercialData, allVideos, selectedVideos, beautyVideos, videoBuffers) {
    ...
    **5. 美妆护肤专项统计（由后端计算，供你引用）:**
    ...
    ## 二、美妆护肤类目专项分析（请基于注入数据计算并填充下列指标，并为每项追加“指标解释”）
    - 概览与垂直度
    ...
    - 表现与对比（美妆 vs 全体）
      - 播放：均值/中位数/P90（分别给出两列对比）（指标解释：整体与头部表现分布，P90代表顶尖表现阈值）
      - 互动率ER = (点赞+评论+分享+收藏)/播放（两列对比）（指标解释：互动质量与受欢迎程度，越高越好）
      - 爆款占比（美妆）：播放 > 全体P90 或 > 全体均值+2σ 的比例（指标解释：产出爆款的能力与频率）
    - 稳定性
      - 播放与ER的变异系数CV（标准差/均值）（指标解释：波动大小，越低越稳定）
      - 近30天/90天滚动波动率（rolling_std/rolling_mean）（指标解释：短中期表现是否稳定）
      - 周发帖稳定性（周频率的标准差；缺更周占比）（指标解释：更新是否规律，缺更风险）
    - 趋势
      - 近90天播放与ER的线性趋势斜率（β）与 Spearman ρ（上升/下降/持平）（指标解释：趋势方向与单调性）
    - 子类与题材（可从描述/话题词提取）
