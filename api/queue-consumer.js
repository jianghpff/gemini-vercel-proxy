...
async function generateStructuredAnalysis(ai, commercialData, allVideos, selectedVideos, beautyVideos, videoBuffers) {
    ...
    **4. 全部美妆护肤类视频数据:**
    ...
    ## 二、美妆护肤类目专项分析（请基于注入数据计算并填充下列指标）
    - 概览与垂直度
      - 美妆内容占比：${beautyVideos.length} / ${allVideos.length} (${(allVideos.length > 0 ? (beautyVideos.length / allVideos.length) * 100 : 0).toFixed(1)}%)
      - 近30天/90天美妆发帖频率（条/周）：[分别计算]
      - 垂直度评估：基于占比、描述关键词一致性、子类集中度，给出1–5评分与简要理由
    - 表现与对比（美妆 vs 全体）
      - 播放：均值/中位数/P90（分别给出两列对比）
      - 互动率ER = (点赞+评论+分享+收藏)/播放（两列对比）
      - 爆款占比（美妆）：播放 > 全体P90 或 > 全体均值+2σ 的比例
    - 稳定性
      - 播放与ER的变异系数CV（标准差/均值）
      - 近30天/90天滚动波动率（rolling_std/rolling_mean）
      - 周发帖稳定性（周频率的标准差；缺更周占比）
    - 趋势
      - 近90天播放与ER的线性趋势斜率（β）与 Spearman ρ（上升/下降/持平）
    - 子类与题材（可从描述/话题词提取）
      - 子类TOP3（如护肤/清洁/防晒/彩妆），各自占比与表现（播放均值、ER）
    - 商业意图与转化代理
      - CTA占比（含“购买/链接/折扣/店铺”等）
      - 重复单品信号：近90天同款/同品牌提及≥3次的次数
    - 与我方产品契合度
      - 目标受众匹配度（人群画像 vs 品牌目标）
      - 风格与调性匹配度（1–5评分，含理由）
    ...
    - 对每条视频请输出：
      ...
      - 情绪与语气（energetic/calm/warm/professional/neutral），并给1–5主观强度评分
      - 目标受众画像：性别/年龄段/肤质/预算层级（根据画面与文案合理推断）
      - 脚本结构（时间节点）：Hook(0–3s)、介绍(3–10s)、演示(中段)、证明(中后段)、CTA(收尾)，逐段概述内容与亮点
